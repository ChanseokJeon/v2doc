...
analyze.post(
  ...
  async (c) => {
    ...
    try {
      ...
      const hasSubtitles = metadata.availableCaptions.some(
        (cap) => !cap.isAutoGenerated
      );
      const needsWhisper = !hasSubtitles;
      const whisperCost = needsWhisper
        ? CostEstimator.estimate(metadata.duration).whisperCost
        : 0;
